...
def calculate_wb_from_patch(
    ...
    # Calculate robust mean of the patch
    mean_rgb = _robust_mean(patch)
    eps = 1e-6

    if method == "gray_world":
        # Target: all channels equal to perceived luminance
        perceived_lum = _perceived_luminance(mean_rgb)
        gains_raw = perceived_lum / (mean_rgb + eps)

        # Clamp to reasonable range
        gains = np.clip(gains_raw, 0.5, 3.0)

    elif method == "max_white":
        # Target: all channels equal to brightest channel
        max_channel = float(mean_rgb.max())
        gains_raw = max_channel / (mean_rgb + eps)
