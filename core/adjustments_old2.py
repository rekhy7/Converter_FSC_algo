...
def apply_all_adjustments(
    ...
    FIXES:
    - Exposure: Stronger and more uniform
    - Saturation: HSV-based, preserves luminance
    - CMY: Fixed sign inversion
    - Shadows: Better algorithm without color shifts
    """
    ...
    # ========================================================================
    # Step 1: Exposure (in linear space - STRONGER)
    # ========================================================================
    if abs(exposure) > 1e-6:
        # Make exposure more aggressive
        exposure_factor = 2.0 ** exposure
        img = np.clip(img * exposure_factor, 0.0, 1.0)

    # ========================================================================
    # Step 2: Gamma (in linear space)
    # ========================================================================
    if abs(gamma - 1.0) > 1e-6:
        img = np.clip(np.power(img, 1.0 / gamma), 0.0, 1.0)
    ...
    # ========================================================================
    # Step 5: Saturation (HSV-based to preserve luminance) - FIXED
    # ========================================================================
    if abs(saturation - 1.0) > 1e-6:
        img_srgb = _apply_saturation_hsv(img_srgb, saturation)

    # ========================================================================
    # Step 6: CMY (luma-neutral) - FIXED SIGNS
    # ========================================================================
    if abs(cyan) > 1e-3 or abs(magenta) > 1e-3 or abs(yellow) > 1e-3:
        img_srgb = _apply_cmy_luma_neutral(img_srgb, cyan, magenta, yellow)

    # ========================================================================
    # Step 7: Shadows & Highlights - IMPROVED
    # ========================================================================
    if abs(shadows) > 1e-3 or abs(highlights) > 1e-3:
        img_srgb = _apply_shadows_highlights_improved(img_srgb, shadows, highlights)

    # ========================================================================
    # Step 8: Black & White Point - DISABLED (too aggressive)
    # ========================================================================
    # Commented out - not working well yet
    # if abs(blackpoint) > 1e-3 or abs(whitepoint) > 1e-3:
    #     img_srgb = _apply_black_white_point(img_srgb, blackpoint, whitepoint)

    # ========================================================================
    # Step 9: Convert back to linear
    # ========================================================================
...
def _apply_cmy_luma_neutral(
    ...
    # FIX: Corrected signs (removed negation that was in old version)
    # +cyan should reduce red, +magenta should reduce green, +yellow should reduce blue
    gain_r = 1.0 - strength * cyan      # +cyan → less red
    gain_g = 1.0 - strength * magenta   # +magenta → less green
    gain_b = 1.0 - strength * yellow    # +yellow → less blue
